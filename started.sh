...
######### Comprueba el numero de errores
if [ -f /var/log/atras.txt ]; then
echo "Fichero contador -------------------------- $colorverde"
else
echo "Fichero contador no existe ---------------- $colorrojo"
echo "Creando.."
echo " "
touch /var/log/atras.txt
chmod 755 /var/log/atras.txt
fi
...
sleep 2; ping -c 1 10.8.0.12 &>/var/log/server_ping.txt 

pingvalor=$(echo $?)

if [ $pingvalor -eq 0 ]; then
echo "Conexion realizada ----------------------------------- $colorverde"
echo "$pingvalor" >> /var/log/atras.txt
else
...
error=$(cat /var/log/atras.txt | grep 1 | wc -l)
...
fdtest=$(ps awx | grep bacula-fd | grep -v grep | wc -l)
if [ $fdtest -eq 0 ]; then
systemctl start bacula-fd
##
fdtest1=$(ps awx | grep bacula-fd | grep -v grep | wc -l)
if [ $fdtest1 -eq 1 ]; then
echo "El servicio Bacula-fd se ha ejecutado correctamente -------------- $colorverde"
else
echo "El servicio Bacula-fd no se ha ejecutado correctamente ----------- $colorrojo"
fi
##
else
echo "El servicio bacula-fd estaba encendido cuando no debia [Script: started.sh]"
let recuento_fallos_procesos=$recuento_fallos_procesos+1
echo "El servicio bacula-fd estaba encendido cuando no debia [Script: started.sh] $fecha" >> /var/log/logs_comprobador
fi
...
if [[ $recuento_fallos_procesos -eq 0 ]]; then
valor=1
fi
