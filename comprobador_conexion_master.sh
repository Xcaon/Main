...
if [ $estado -eq 0 ]; then
echo "El servicio bacula-director se ha apagado correctamente -------------------- $colorverde" 
let contador_servicios=$contador_servicios+1
else
echo "El servicio bacula-director no se ha apagado correctamente ------ $colorrojo"
echo "El servicio bacula-director no se ha apagado correctamente [script: comprobador_conexion_master.sh]" >> /var/log/logs_comprobador
fi

systemctl stop bacula-sd
estado1=$(ps awx | grep bacula-sd | grep -v grep | wc -l)
if [ $estado1 -eq 0 ]; then
echo "El servicio bacula-sd se ha apagado correctamente ------------------- $colorverde"
let contador_servicios=$contador_servicios+1
else
...
if [[ $suma -eq 0 ]]; then
echo "No ha sido posible probar conectividad con el SV-master ---- $colorinfo"
fi
